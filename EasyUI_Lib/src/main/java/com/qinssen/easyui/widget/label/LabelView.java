package com.qinssen.easyui.widget.label;
 ...
 * 标签列表
 * <p>
 * <com.qinssen.easyui.widget.label.LabelsView
...
public class LabelView extends ViewGroup implements View.OnClickListener {
    ...
    private ArrayList<Object> mlabel = new ArrayList<>();
    //保存选中的label的位置
    private ArrayList<Integer> mSelectlabel = new ArrayList<>();

    //保存必选项。在多选模式下，可以设置必选项，必选项默认选中，不能反选
    private ArrayList<Integer> mCompulsorys = new ArrayList<>();

    private OnLabelClickListener mLabelClickListener;
    private OnlabelelectChangeListener mlabelelectChangeListener;
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            int type = mTypedArray.getInt(R.styleable.LabelView_selectType, 1);
            mSelectType = SelectType.get(type);

            mMaxSelect = mTypedArray.getInteger(R.styleable.LabelView_maxSelect, 0);
            mMaxLines = mTypedArray.getInteger(R.styleable.LabelView_maxLines, 0);
            mTextColor = mTypedArray.getColorStateList(R.styleable.LabelView_labelTextColor);
            mTextSize = mTypedArray.getDimension(R.styleable.LabelView_labelTextSize,
                    sp2px(context, 14));
            mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelView_labelTextPaddingLeft, 0);
            mTextPaddingTop = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelView_labelTextPaddingTop, 0);
            mTextPaddingRight = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelView_labelTextPaddingRight, 0);
            mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(
                    R.styleable.LabelView_labelTextPaddingBottom, 0);
            mLineMargin = mTypedArray.getDimensionPixelOffset(R.styleable.LabelView_lineMargin, 0);
            mWordMargin = mTypedArray.getDimensionPixelOffset(R.styleable.LabelView_wordMargin, 0);
            int labelBgResId = mTypedArray.getResourceId(R.styleable.LabelView_labelBackground, 0);
            if (labelBgResId != 0) {
                mLabelBg = getResources().getDrawable(labelBgResId);
            } else {
                int labelBgColor = mTypedArray.getColor(R.styleable.LabelView_labelBackground, Color.TRANSPARENT);
    ...
    // 最终由LabelTextProvider提供，所以labelView不再在onSaveInstanceState()和onRestoreInstanceState()
    // 中保存和恢复标签列表的数据。
    private static final String KEY_label_STATE = "key_label_state";
    private static final String KEY_SELECT_label_STATE = "key_select_label_state";
    private static final String KEY_COMPULSORY_label_STATE = "key_select_compulsory_state";
...
//        bundle.putInt(KEY_BG_RES_ID_STATE, mLabelBgResId);
        ...
        //保存标签列表
//        if (!mlabel.isEmpty()) {
//            bundle.putStringArrayList(KEY_label_STATE, mlabel);
//        }
        //保存已选择的标签列表
        if (!mSelectlabel.isEmpty()) {
            bundle.putIntegerArrayList(KEY_SELECT_label_STATE, mSelectlabel);
        }

        //保存必选项列表
        if (!mCompulsorys.isEmpty()) {
            bundle.putIntegerArrayList(KEY_COMPULSORY_label_STATE, mCompulsorys);
        }
...
//            //恢复标签列表
//            ArrayList<String> label = bundle.getStringArrayList(KEY_label_STATE);
//            if (label != null && !label.isEmpty()) {
//                setlabel(label);
//            }
            //恢复必选项列表
            ArrayList<Integer> compulsory = bundle.getIntegerArrayList(KEY_COMPULSORY_label_STATE);
            if (compulsory != null && !compulsory.isEmpty()) {
                setCompulsorys(compulsory);
            }
            //恢复已选择的标签列表
            ArrayList<Integer> selectLabel = bundle.getIntegerArrayList(KEY_SELECT_label_STATE);
    ...
    public void setlabel(List<String> label) {
        setlabel(label, new LabelTextProvider<String>() {
            @Override
     ...
     * @param <T>
     */
    public <T> void setlabel(List<T> label, LabelTextProvider<T> provider) {
        //清空原有的标签
        innerClearAllSelect();
        removeAllViews();
        mlabel.clear();
        ...
        if (label != null) {
            ...
            int size = label.size();
            for (int i = 0; i < size; i++) {
                addLabel(label.get(i), i, provider);
            }
     ...
     * @return
     */
    public <T> List<T> getlabel() {
        return (List<T>) mlabel;
    }
    ...
    private void setlabelelect(TextView label, boolean isSelect) {
        if (label.isSelected() != isSelect) {
            label.setSelected(isSelect);
            if (isSelect) {
                mSelectlabel.add((Integer) label.getTag(KEY_POSITION));
            } else {
                mSelectlabel.remove((Integer) label.getTag(KEY_POSITION));
            }
            if (mlabelelectChangeListener != null) {
                mlabelelectChangeListener.onlabelelectChange(label, label.getTag(KEY_DATA),
    ...
    private void innerClearAllSelect() {
        int count = getChildCount();
        for (int i = 0; i < count; i++) {
            setlabelelect((TextView) getChildAt(i), false);
        }
        mSelectlabel.clear();
    }
    ...
    public void setSelects(int... positions) {
        if (mSelectType != SelectType.NONE) {
            ArrayList<TextView> selectlabel = new ArrayList<>();
            int count = getChildCount();
            ...
            for (int p : positions) {
                if (p < count) {
                    ...
                    if (!selectlabel.contains(label)) {
                        setlabelelect(label, true);
                        selectlabel.add(label);
                    }
                    if (size > 0 && selectlabel.size() == size) {
                        break;
                    }
                }
            }
     ...
     * @return
     */
    public List<Integer> getSelectlabel() {
        return mSelectlabel;
    }
    ...
    public <T> List<T> getSelectLabelDatas() {
        ...
        int size = mSelectlabel.size();
        for (int i = 0; i < size; i++) {
            View label = getChildAt(mSelectlabel.get(i));
     ...
     * @param l
     */
    public void setOnlabelelectChangeListener(OnlabelelectChangeListener l) {
        mlabelelectChangeListener = l;
    }
    ...
    public interface OnlabelelectChangeListener {
         ...
         * @param position 标签位置
         */
        void onlabelelectChange(TextView label, Object data, boolean isSelect, int position);
    }

    /**
     * 给标签提供最终需要显示的数据。因为labelView的列表可以设置任何类型的数据，而labelView里的每个item的是一
